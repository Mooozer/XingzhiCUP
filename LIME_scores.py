...
# Prepare test data for LIME interpretation
sentence_with_space = []
...
# Run LIME to get LIME scores
output_dir = './LSweight_macbert_large1/' 读取新训练的模型
model_interpret = BertForSequenceClassification.from_pretrained(output_dir)
def predictor(texts):
...
def lime_obtain_output_dic(LCQMC_test, bow = False):
    ...
    for ID in LCQMC_test['id']:
        ...
        number += 1 
    if not os.path.exists('lime_output'):
        os.mkdir('lime_output')
    np.save('lime_output/output.npy', output) 
...
# 读取训练完成的模型的lime得分
lime_output = np.load('./lime_outputB/output.npy',allow_pickle=True).item()
