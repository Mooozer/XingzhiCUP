...
# Training three models, epoch = 3, 2, 3 respectively.
epochs_list = [3,2,3]
for i in range(len(epochs_list)):
    ...
    #save models
    output_dir = './LSweight_macbert_large' + str(i+1) +'/'
...
# Model 1 (macbert_large1 + LW, epoch=3) prediction and save logits
...
for batch in test_dataloader:        
    ...
    validation_time = format_time(time.time() - t0)
    print(batch, validation_time)
    
if not os.path.exists('predictionB'):
    os.mkdir('predictionB')
np.save('predictionB/logits13_B_test.npy', np.array(logits_B_test)) 
...
# Model 2 (macbert_large2 + LW, epoch=2)   prediction and save logits
...
if not os.path.exists('predictionB'):
    os.mkdir('predictionB')
np.save('predictionB/logits22_B_test.npy', np.array(logits_B_test)) 


########################################################

# Model 3 (macbert_large3 + LW, epoch=3) prediction and save logits
...
if not os.path.exists('predictionB'):
    os.mkdir('predictionB')
np.save('predictionB/logits33_B_test.npy', np.array(logits_B_test)) 
